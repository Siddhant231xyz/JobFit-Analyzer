...
def is_question_relevant(question: str) -> bool:
    # Define keywords that indicate a question is in-context
    keywords = ["resume", "job", "jd", "candidate", "application", "experience", "qualification", "cv"]
...
def query_with_context(question: str, context_docs: list) -> str:
    ...
    if not is_question_relevant(question):
        return "Out-of-context question. Please ask questions related to the resumes or the job description."
...
if "initialized" not in st.session_state:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        st.session_state.OPENAI_API_KEY = st.text_input("Enter API key for OpenAI:", type="password")
    ...
    # --- Load Resumes from a Mounted Volume ---
    # Here we assume the resumes are stored in the mounted folder /data/resume
